...
metrics = ["arel_res_public.csv", "cp_res_public.csv", "crec_res_public.csv", 
    ...
    for run in range(1, 11):  # Assuming folders are named from 1 to 10
        ...
        for metric in metrics:
            ...
            if os.path.exists(file_path):
                ...
                num_rows = len(df) if len(df) > 0 else 1  # Avoid division by zero
...
plt.tight_layout()
plt.savefig("Comparison_Metrics_public.pdf", bbox_inches="tight")
plt.show()
