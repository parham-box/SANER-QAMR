...
def load_basic_retriever(k,sl=None,dt=None):
  print("Function: load_basic_retriever \t Status: Started") 
  ...
  retriever1 = vectorstore.as_retriever(
      search_type="mmr", search_kwargs={"k": k, "include_metadata":True}
  )
  ...
  print("Function: load_basic_retriever \t Status: Completed")  
...
def embed_documents(device, splits, k,sl):
  ...
  for batch in chunks(splits, 41600):
  ...
  print("Function: embed_documents \t Status: Completed")  
...
def get_retrievers(type, chunk_size=0, child_chunk_size=0, chunk_overlap=0,child_chunk_overlap=0,k=3, extra_context_overlap=0):
  ...
  if load_presist == 'n':
    ...
    for dt in documen_corpus_type:
      ...
      doc_splits=[]
      retriever=None
      if type == RetrieverType.PAGE_CHUNK_BASIC_RET:
        print("Function: Get RetrieverType.PAGE_CHUNK_BASIC_RET \t Status: Started")  
        #Basic retriever, chunk by pages and char limit if page is too large
        for document in documents:
          #split doc to chunks and add to doc_splits
          doc_splits.extend(split_documents(document=document, chunk_size=chunk_size, chunk_overlap=chunk_overlap))
        free_gpu_memory()
        #get basic retriever
        retriever = embed_documents(device, doc_splits, k,sl=source_location)
        print("Function: Get RetrieverType.PAGE_CHUNK_BASIC_RET \t Status: Completed")  
    ...
    print("Function: get_retriever \t Status: Completed")  
    return [load_basic_retriever(k=2,sl=dt['document_location'],dt=dt) for dt in documen_corpus_type]
