...
class RAG():
    ...
    def retrieve_docs_for_questions(self,retriever,questions,sa,version,bc):
        ...
        fc_chain1 = ({"context":itemgetter("context"), "question":itemgetter("question"), "version":itemgetter("version")} 
        | PromptTemplate(
        template = answer_extraction_llama3,
        input_variables = ["context", "question","version"]) 
        | self.llm 
        | StrOutputParser()
        )
        ...
        with open('results_requesta/docs.txt', 'a') as file:
            ...
            df = self.answer_extraction(unique_docs,version,sa,fc_chain1,fc_chain11)
    ...
    def answer_extraction(self, unique_docs, version, sa,fc_chain1,fc_chain11):
        start_time_context_reduction = time.time()
        dff = []
        ...
        # List of inputs
        inputs = [{"context": doc['doc'].page_content, "question": sa, "version": version} for doc in unique_docs]
        batch_responses1 = []
        with open('results_requesta/docs.txt', 'a') as file:
            for batch in chunk_list(inputs, 4):
                responses = fc_chain1.batch(batch)
                batch_responses1.extend(responses)
            
            file.write(f"RESULTS: \n {batch_responses1} \n")
            answers = [parse_json_garbage('{'+ resp ) for resp in batch_responses1]
            answers = [ans['extracted_text'] for ans in answers if ans != 'None']
            end_time_context_reduction = time.time()
            context_reduction_times.append(end_time_context_reduction - start_time_context_reduction)
            start_time_context_selection = time.time()
            for doc, ress in zip(unique_docs, answers):
                if(len(ress) > 0):
                    resss = "\n".join(ress)
                    dff.append({'doc': Document(page_content=resss, metadata= doc['doc'].metadata), 'q': doc['q']})
                    file.write(f"RESPONSES: \n{doc['doc'].metadata} - {ress}\n")
            cts = format_contexts([doc['doc'] for doc in dff])
            # dfff = []
        ...
        dff = [dff[id] for id in first_three_ids if id >= 0  and id < len(dff)]    
        ...
        # return [dff[id] for id in answers if id < len(dff)]
        return dff
